...
import { pharmacistAddEditSchema, type PharmacistAddEditFormType } from "../../../../../../services/zod_schema/manage_staff/pharmacist/zodPharmacistAddEditSchema";
import { usePharmacistCreate } from "../../../../../../hooks/query_hooks/pharmacistHooks";
...
export const PharmacistAddEditForm = () => {
  const { state } = useLocation()
  const pharmacistObject = pharmacistAddEditSchema();
  ...
  const topBarAndFilter = {
    type: "pharmacist",
    heading: mode == 'create' ? "Create Pharmacist" : "Edit Pharmacist",
    viewMode: "grid",
    whatNeeded: {
      isBackNeeded: true,
    },
  };
  const form = useFormHelper<PharmacistAddEditFormType>({
    initialValues: {
      ...
      state: state ? 'TX' : '',
      pinCode: state ? '78701' : '',
      about: state ? 'I am a pharmacist' : '',
    },
    validationSchema: pharmacistObject,
    mode: 'controlled',
  });
  ...
  const { mutate: createPharmacist, status } = usePharmacistCreate(() => {
  ...
  const onSubmitCall = (formData: PharmacistAddEditFormType) => {
    ...
    try {
      ...
      pharmacistObject.parse(submitData);
      createPharmacist(submitData);
  ...
  return (
    <Box>
      <Box>
        <Breadcrumb
          dataPass={{
            pageTitle: mode == 'create' ? "Create Pharmacist" : "Edit Pharmacist",
            items: [
              { title: "Manage Pharmacists", href: "#" },
              { title: "Pharmacist List", href: "/medical-providers/pharmacist" },
              { title: mode == 'create' ? "Create Pharmacist" : "Edit Pharmacist", href: "#", isActive: true },
            ],
          }}
        />
      </Box>
        ...
        <Card withBorder shadow="sm" className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl border-t border-slate-200/60 dark:border-slate-700/60 rounded-xl">
          ...
          <Box className="p-3">
            <FormWrapper
              ...
              submitButtonText={mode === "create" ? "Create pharmacist" : "Update pharmacist"}
              isLoading={isLoading}>
              <Grid>
