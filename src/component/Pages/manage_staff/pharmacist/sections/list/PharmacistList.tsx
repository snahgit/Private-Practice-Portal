...
import { IconDetails, IconDots, IconEdit, IconInfoCircle, IconMailPin, IconPhone } from "@tabler/icons-react";
...
import { PharmacistQuickView } from "../detail/PharmacistQuickView";
...
export const PharmacistList = (props: { dataPass: any }) => {
  const { pharmacistList } = props.dataPass;
  ...
  const endIndex = Math.min(pagination.endIndex, pharmacistList.length);
  const currentData = pharmacistList.slice(startIndex, endIndex);
  ...
  useEffect(() => {
    if (pharmacistList && pharmacistList.length > 0) {
      dispatch(
        setPagination({
          totalItems: pharmacistList.length,
          itemsPerPage: 12,
          currentPage: pagination.currentPage || 1,
        })
      );
    }
  }, [pharmacistList, dispatch]);
    ...
    <>
      <PageDrawer
        ...
        dataPass={{
          ...
          component: <PharmacistQuickView dataPass={{ pharmacistInfo }} />,
          title: 'Pharmacist Quick Information',
        }}
      />
