...
import { WalletTransactionList } from "./sections/list/WalletTransactionList";
...
export const WalletTransaction = () => {
  ...
  const topBarAndFilter = {
    ...
    heading: "List of all wallet transactions you can find below",
    viewMode: "grid",
    whatNeeded: {
  ...
  const paymentsLogList = useMemo(() => {
    ...
    if (filterState.filter.walletFilter && filterState.filter.walletFilter !== 'all') {
      filtered = filtered.filter(dept =>
        dept.status.toLowerCase() === filterState.filter.walletFilter.toLowerCase()
      );
    }
    return filtered;
  ...
  return (
    <Box>
      <Box>
        <Breadcrumb
          dataPass={{
            pageTitle: "Wallet Transaction List",
            items: [
              { title: "Transaction History", href: "#" },
              { title: "Wallet Transaction List", href: "#", isActive: true },
            ],
          }}
        />
      </Box>
      <Box>
        <Card withBorder shadow="sm" className="rounded-xl bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl border-t border-slate-200/60 dark:border-slate-700/60">
          ...
          <WalletTransactionList dataPass={{ paymentsLogList }} />
