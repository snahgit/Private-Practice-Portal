...
import PaymentsLogList from "./sections/payments_log_list/PaymentsLogList";
...
export const ClaimTransaction = () => {
  ...
  const topBarAndFilter = {
    ...
    heading: "List of all claim transactions you can find below",
    viewMode: "grid",
    whatNeeded: {
  ...
  const paymentsLogList = useMemo(() => {
    let filtered = [...allPaymentsLogList];
      ...
      filtered = filtered.filter(dept =>
        dept.patientName.toLowerCase().includes(textFilter)
      );
    return filtered;
  ...
  return (
    <Box>
      <Box>
        <Breadcrumb
          dataPass={{
            pageTitle: "Claim Transaction List",
            items: [
              { title: "Transaction History", href: "#" },
              { title: "Claim Transaction List", href: "#", isActive: true },
            ],
          }}
        />
      </Box>
